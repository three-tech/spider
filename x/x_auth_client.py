            ...
            if res_cookies:
                ...
                for cookie_part in cookie_parts:
                    ...
                    if '=' in main_part:
                        ...
                        value = value.strip()
                        # 过滤掉无效的cookie属性
                        if name and not name.lower().startswith(('path', 'domain', 'expires', 'max-age', 'secure', 'httponly', 'samesite')):
                    ...
                    if 'data' in data and 'user' in data['data'] and 'result' in data['data']['user']:
                        ...
                        if len(current_users) == 0:
                            ...
                            if len(current_users) < 20:  # 每页期望20个用户
                                logging.info(f"⏹️ 终止原因：返回用户数({len(current_users)})少于预期(20)，已获取完所有数据")
                                break
